...
public interface IZoneController extends IBaseController {
    @Override
    ...
    @PostMapping
    ResponseEntity<?> save(@Valid @RequestBody ZoneSaveReq req, Errors errors);

    @PutMapping("/{id}")
    ResponseEntity<?> update(@Valid FindByIdReq reqId, @RequestBody ZoneSaveReq req, Errors errors);

    @PostMapping("/batch")
    ResponseEntity<?> saveAll(@Valid @RequestBody List<ZoneSaveReq> reqs, Errors errors);
